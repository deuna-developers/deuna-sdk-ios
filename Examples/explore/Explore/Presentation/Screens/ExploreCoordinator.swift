...
final class ExploreCoordinator: ObservableObject {
    ...
    @Published var isLaunchingWallets = false
    @Published var isLaunchingFormularios = false
    @Published var apmOptions: [ApmOption] = []
    @Published var isLoadingApms = false
    ...
    @Published var generatedOrderToken: String?
    ...
    func refreshEmbedded() {
        guard appliedConfig.presentationMode == .embedded, isShowingEmbeddedScreen else { return }
    ...
    func showModalWidget() async {
        ...
        if output.modalStatusMessage == nil {
            secureSettingsRepository.saveConfiguration(appliedConfig)
            if !useManualOrderTokenFlow, !appliedConfig.orderToken.isEmpty {
                generatedOrderToken = appliedConfig.orderToken
            }
        }
    }

    func clearGeneratedOrder() {
        appliedConfig.orderToken = ""
        draftConfig = ExploreDraftConfig(from: appliedConfig)
        generatedOrderToken = nil
    ...
    /// Tokenizes a cart order (if needed) then navigates to WalletsScreen.
    @MainActor
    func showWallets() async {
        modalStatusMessage = nil
        isLaunchingWallets = true
        defer { isLaunchingWallets = false }

        var orderToken: String? = appliedConfig.orderToken.isEmpty ? nil : appliedConfig.orderToken
        ...
        if !useManualOrderTokenFlow {
            ...
            let privateKey = appliedConfig.privateKey.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            do {
                ...
                let result = try await paymentsRepository.createOrderToken(
                    environment: appliedConfig.environment,
                    privateKey: privateKey,
                    products: selectedProducts
                )
                appliedConfig.orderToken = result.orderToken
                appliedConfig.merchantName = result.merchantProfile.name
                appliedConfig.merchantCountryCode = result.merchantProfile.countryCode
                appliedConfig.merchantCurrencyCode = result.merchantProfile.currencyCode
                draftConfig = ExploreDraftConfig(from: appliedConfig)
        ...
        let definitions = WidgetDefinitions()
        let userInfo = definitions.fallbackUserInfoIfNeeded(appliedConfig)
        navigationPath.append(ExploreRoute.wallets(deunaSDK: deunaSDK, orderToken: orderToken, userInfo: userInfo))
    }

    func loadApmOptions() {
        guard !isLoadingApms else { return }
        isLoadingApms = true
        apmOptions = []
        Task { @MainActor in
            do {
                apmOptions = try await ApmRepository().fetchIosCompatible()
            } catch {
                apmOptions = []
            }
            isLoadingApms = false
    ...
    @MainActor
    func showFormularios(apm: ApmOption) async {
        modalStatusMessage = nil
        isLaunchingFormularios = true
        defer { isLaunchingFormularios = false }

        let prep = await widgetLaunchHandler.prepare(
            input: .init(
                appliedConfig: appliedConfig,
                draftConfig: draftConfig,
                products: products,
        ...
        guard prep.modalStatusMessage == nil else { return }

        if appliedConfig.presentationMode == .modal {
            paymentsRepository.showFormulariosModal(config: appliedConfig, apm: apm)
        } else {
            embeddedWidgetConfig = paymentsRepository.makeFormulariosEmbeddedConfig(config: appliedConfig, apm: apm)
