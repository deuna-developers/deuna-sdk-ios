...
extension DeunaSDK {
    ...
    public func onWidgetClosed() {
        guard let controller = checkoutWebViewController ?? paymentWidgetViewController ?? elementsWebViewController ?? nextActionViewController ?? voucherViewController else {
            return
        }
        ...
        }else if let voucher = voucherViewController {
            voucher.callbacks.onClosed?(controller.closeAction)
        } else if let nextAction = nextActionViewController {
        ...
        voucherViewController = nil
        nextActionViewController = nil
