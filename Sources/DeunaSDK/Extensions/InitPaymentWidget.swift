...
public extension DeunaSDK {
    ...
    func initPaymentWidget(
        ...
        if cssFile != nil, !cssFile!.isEmpty {
            queryParameters.append(URLQueryItem(name: QueryParameters.cssFile, value: cssFile))
        }
    ...
    @available(*, deprecated, message: "Use setCustomStyle instead.")
    ...
    /// - Parameters:
    ///   - data: The JSON data to update the payment widget UI
    func setCustomStyle(data: [String: Any]) {
        paymentWidgetViewController?.setCustomStyle(data: data)
    }
}
