...
public class DeunaSDKConfiguration: NSObject {
...
public class DeunaSDK: NSObject {
    ...
    public let configuration: DeunaSDKConfiguration
    ...
    public init(environment: Environment, publicApiKey: String, useMainThread: Bool = false) {
        ...
        self.configuration = DeunaSDKConfiguration(environment: environment, publicApiKey: publicApiKey)
    }
    ...
    func overrideBaseUrl(baseUrl: String, replaceWith: String) -> String {
        ...
        if replaceWith.hasPrefix("http://") || replaceWith.hasPrefix("https://") {
            let originalHost = url.port != nil ? "\(host):\(url.port!)" : host
            let originalPrefix = "\(scheme)://\(originalHost)"
            
            return baseUrl.replacingOccurrences(of: originalPrefix, with: replaceWith)
        }
        
        // For domain without protocol, preserve original protocol and port
        let originalHost = url.port != nil ? "\(host):\(url.port!)" : host
        return baseUrl.replacingOccurrences(of: originalHost, with: replaceWith)
    }
}
