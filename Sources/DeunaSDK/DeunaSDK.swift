...
public class DeunaSDK: NSObject {
    ...
    public func setCustomStyle(data: [String: Any]) {
        if let controller = paymentWidgetViewController ?? checkoutWebViewController ?? elementsWebViewController ?? nextActionViewController ?? voucherViewController {
    ...
    public func submit(completion: @escaping (SubmitResult) -> Void) {
        if let paymentWidgetController = paymentWidgetViewController {
            
            paymentWidgetSubmitStrategy(completion: completion)
            
        } else if let controller = checkoutWebViewController ?? elementsWebViewController {
