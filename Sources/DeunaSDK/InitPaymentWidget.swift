...
public extension DeunaSDK {
    ...
    func initPaymentWidget(
        ...
        paymentWidgetViewController = PaymentWidgetViewController(
            callbacks: callbacks,
    ...
    func paymentWidget(
        ...
        if let urlString = paymentWidgetURL {
            paymentWidgetViewController?.loadUrl(urlString: urlString)
            DeunaLogs.debug("Loading payment widget link: \(urlString)")
        }
        return DeunaWebViewRepresentable(webViewController: paymentWidgetViewController!)
    }
    ...
    private func buildPaymentWidgetURL(
        ...
        let baseUrl = "\(environment.config.paymentWidgetBaseUrl)/now/\(orderToken)"
