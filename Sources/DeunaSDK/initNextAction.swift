...
extension DeunaSDK {
    public func initNextAction(
        ...
        nextActionViewController = NextActionViewController(callbacks: callbacks)
        ...
        if !showWebView(webViewController: nextActionViewController!) {
            return
        }
        ...
        nextActionViewController?.loadUrl(urlString: link)
    }
    ...
    func nextActionWidget(
        ...
        nextActionViewController = NextActionViewController(
        ...
        if let urlString = url {
            nextActionViewController?.loadUrl(urlString: urlString)
            DeunaLogs.debug("Loading payment widget link: \(urlString)")
        }
        return DeunaWebViewRepresentable(webViewController: nextActionViewController!)
    }
