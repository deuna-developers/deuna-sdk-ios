...
extension DeunaSDK: UIAdaptivePresentationControllerDelegate {
    ...
    internal func showCloseButtonInView() {
        if canDismiss()  && showCloseButton{
            ...
            self.DeunaElementsWebView?.addSubview(closeButton)
            self.DeunaElementsWebView?.bringSubviewToFront(closeButton)
        }
    }
    ...
    internal func hideCloseButtonInView() {
        ...
        if let elementsCloseButton = self.DeunaElementsWebView?.viewWithTag(closeButtonTag) {
            elementsCloseButton.removeFromSuperview()
        }
    }
