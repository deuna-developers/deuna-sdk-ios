...
extension PaymentWidgetViewController {
    public func handleEventData(message: WebViewController.Message) {
        ...
        case .purchase:
            externalUrlHandler.closeExternalUrlWebView()
            guard let order = data["order"] as? Json else {
                callbacks.onError?(
                    PaymentsError(
                        type: .unknownError,
                        metadata: PaymentsError.ErrorMetadata(
                            code: ErrorCodes.unknown,
                            message: "Invalid purchase payload: missing order"
                        )
                    )
                )
                return
            }
            buildSuccessPayload(order) { enrichedPayload in
