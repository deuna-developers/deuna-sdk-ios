...
public extension DeunaSDK {
    ...
    ///     )
    ///     ```
    ///   - fraudCredentials: (Optional) Credentials used to generate the fraud id.
    ///   - customUserAgent: (Optional) Custom user agent to be used by the webview.
    ...
    func initElements(
        ...
        let requestedProvider = types.compactMap { entry -> WalletProvider? in
            guard let name = entry["name"] as? String else { return nil }
            return WalletProvider(rawValue: name.lowercased())
        }.first

        if let provider = requestedProvider {
            WalletElements(
    ...
    private func buildElementsURL(
        ...
        var queryParameters = [
            ...
            (QueryParameters.publicApiKey, self.configuration.publicApiKey),
        ...
        if let userInfo = userInfo {
            ...
            if let firstName = userInfo.firstName {
                queryParameters.append((QueryParameters.firstName, firstName.encodeValue()))
            }
            if let lastName = userInfo.lastName {
                queryParameters.append((QueryParameters.lastName, lastName.encodeValue()))
            }
        ...
        var baseUrl = "\(self.configuration.environment.config.elementsBaseUrl)/\(widgetName)"
