    ...
    public static func fromJson(data: Json) -> PaymentsError? {
        guard
            ...
            return PaymentsError(
                ...
                order: data["order"] as? Json
            )
        }

        let errorCode = metadata["errorCode"] as? String ?? metadata["code"] as? String
        let errorMessage = metadata["errorMessage"] as? String ?? metadata["reason"] as? String
