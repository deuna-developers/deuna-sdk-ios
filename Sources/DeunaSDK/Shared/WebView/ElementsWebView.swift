...
class ElementsViewController: BaseWebViewController, WKScriptMessageHandler {
    ...
    func userContentController(
        ...
        guard messageData.json["type"] as? String !=  ElementsErrorType.vaultFailed.rawValue else {
            self.callbacks.onError?(ElementsError(type: .vaultFailed))
            return
        }
        ...
        do {
            ...
            let eventData = try decoder.decode(ElementsResponse.self, from: messageData.data)
