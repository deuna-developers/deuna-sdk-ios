...
extension DeunaSDK{
    ...
    @objc public func initElements(element: DeunaSDK.ElementType, callbacks: ElementsCallbacks, viewToEmbedWebView: UIView? = nil, userToken: String?) {
        ...
        let environmentUrls = [
            ...
            Environment.sandbox: "https://elements.sandbox.deuna.io/\(element.rawValue)"
        ]
        if let baseUrl = environmentUrls[DeunaSDK.shared.environment],
           let apiKey = apiKey {
            elementURL = "\(baseUrl)?userToken=\(token!)&publicApiKey=\(apiKey)"
        }
        ...
        self.DeunaElementsWebView?.backgroundColor = .clear
